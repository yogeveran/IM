...
class QLearningAgent(ReinforcementAgent):
  ...
  def getQValue(self, state, action):
    ...
    """
      Sets Q(state,action) to be qValue
    """
  ...
  def getPolicy(self, state):
    ...
    bestActions = []
    for act in self.getLegalActions(state):
        if maxQ == self.getQValue(state, act):
  ...
  def update(self, state, action, nextState, reward):
    ...
    newVal = reward + self.gamma * self.getValue(nextState)
    qValue = (1.0 - self.alpha)*oldVal + self.alpha*newVal
...
class ApproximateQAgent(PacmanQAgent):
  ...
  def getQValue(self, state, action):
    ...
    res = 0
    for index, value in features.iteritems():
        res += self.W[index] * value
    return res
  ...
  def update(self, state, action, nextState, reward):
    ...
    features = self.featExtractor.getFeatures(state, action)
    for index, value in features.iteritems():
        self.W[index] += self.alpha * correction * features[index]
